...
const verificarClube = () => {
    if (localStorage.getItem("clube")) {    // se já estiver salvo algum clube
        ...
        let visita = localStorage.getItem("contador");
        visita++;
        localStorage.setItem("contador", visita);
        resp.innerText = `Que bom que você voltou! Esta é a sua visita de número ${visita} ao nosso site.`;
    } else {
        resp.innerText = `Muito Bem-Vindo(a)! Esta é a sua primeira visita ao nosso site.`;
        let contador = localStorage.setItem("contador", 1);
    }
}
